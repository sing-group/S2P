...
public abstract class AbstractSameSpotsDirectoryLoader {
		...
		throws IOException 
	{
		...
		for(File f : listFiles(directory)){
			List<Sample> pair = loadFile(f, threshold);
			pair.forEach(sample -> {
				addSample(sample, samples);
			});
		}
	...
	protected abstract List<Sample> loadFile(File file,
	...
	public static Optional<Sample> findByName(List<Sample> samples,
		String name
	) {
