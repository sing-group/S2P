...
public class SameSpotsReportFileWriterTest {
	...
	private static final Map<String, MascotIdentifications> IDENTIFICATIONS_A =
		new HashMap<>();

	static {
		IDENTIFICATIONS_A.put("276", new MascotIdentifications(asList(
			new MascotEntry("Serum albumin", "A1", 193, 54, 49,	71317.00d, 
			"50ppm_BladderCancer", 5.88d, "ALBU_HUMAN",	TEST_MASCOT_CSV_FILE)
		)));
		IDENTIFICATIONS_A.put("266", new MascotIdentifications(asList(
			new MascotEntry("Serum albumin", "A1", 193, 54, 49,	71317.00d, 
			"50ppm_BladderCancer", 5.88d, "ALBU_HUMAN",	TEST_MASCOT_CSV_FILE)
		)));
	}

	private static final Map<String, MascotIdentifications> IDENTIFICATIONS_B =
			new HashMap<>();

	static {
		IDENTIFICATIONS_B.put("276", new MascotIdentifications(asList(
			new MascotEntry("Serum albumin", "A1", 193, 54, 49,	71317.00d, 
			"50ppm_BladderCancer", 5.88d, "ALBU_HUMAN",	TEST_MASCOT_CSV_FILE)
		)));
	}
